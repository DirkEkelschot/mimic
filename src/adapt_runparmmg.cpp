                                   ...
                                   std::map<int,std::vector<int> > ranges_id)
{
    ...
    std::cout << "nTriangles " << nTriangles << " on rank " << world_rank << std::endl;
    for ( k=0; k<nTriangles; ++k )
    {
        ...
        if(trace_verts.find(tagFaceID)!=trace_verts.end())
        {
            ref = 13;
            for(int s=0;s<3;s++)
            {
                int ref   = 13;
                int tagvid  = trace_verts[tagFaceID][s];
                ...
                std::map<int,int> &new_globE2locE)
{
    ...
    int cnt = 0;
    // std::map<int,int> new_globE2locE;
    std::map<int,int> newtag2traceref;
                ...
                if(testTracePointer != m_PMMG_TraceFacePointer.end())
                {
                    ...
                    if(testTracePrismPointer != m_PMMG_TraceFace2PrismPointer.end())
                    {
                        ...
                        for(int y=0;y<3;y++)
                        {
                            ...
                            if(newtag2traceref.find(vtag)==newtag2traceref.end())
                            {
                                newtag2traceref[vtag]=traceVertRef;
    ...
    for(itmiv=InternalFaces_T.begin();itmiv!=InternalFaces_T.end();itmiv++)
    {
        ...
        {
            nfound++;
            std::cout <<"Found the face -> " << fid << " " << world_rank << " " << lh_T[fid] << std::endl;
        }


        ftot++;
    }
    std::cout << "rh_T " << world_rank << " " << nfound << std::endl; 
    //===================================================================
    ...
    for(itmiv=TraceFaces_T.begin();itmiv!=TraceFaces_T.end();itmiv++)
    {
        ...
        for(int q=0;q<3;q++)
        {
            ...
            if(LocationSharedVert_update.find(itmiv->second[q])!=LocationSharedVert_update.end())
            {
                ...
                int traceRef = newtag2traceref_glob[itmiv->second[q]];
