...
{
    // std::cout << "getAdjacentElementLayer" << std::endl;
    int size;
    MPI_Comm_size(comm, &size);
    // Get the rank of the process
    int rank;
...
void PartObject::buildExtendedAdjacencyDataPerLayer(MPI_Comm comm, 
                                            ...
                                            std::map<int,std::vector<double> > ghosts)
{
    ...
    // std::map<int,std::set<int> > Elems2Check;
    // std::cout << "m_Elem2Vert " << m_Elem2Vert.size() << " " << m_Elem2Elem.size() << std::endl;
    for(its=input_elems.begin();its!=input_elems.end();its++)
    {
        int elid        = *its;
        ...
        for(int j=0;j<nadj;j++)
        {
                ...
                && ghosts.find(adjid)==ghosts.end())
            {
                ...
                //E2A_set[elid].insert(adjid);

                if(m_ElemSet.find(adjid)==m_ElemSet.end() // these guys have been checked already!
                && toCheckset.find(adjid)==toCheckset.end()
                && input_elems.find(adjid)==input_elems.end())
                {
                    toCheck[adjid].insert(elid);  
                    toCheckset.insert(adjid);
                }
                else if(E2A_set.find(elid)!=E2A_set.end())
                {
                    //
                    if(E2A_set[elid].find(adjid)==E2A_set[elid].end())
                    {
                        //std::cout << "we not here. " << std::endl;
                        E2A_set[elid].insert(adjid);
                    }
                }
                // if(toCheck.find(adjid)!=toCheck.end())
                // {
                //     // std::cout << "here" << std::endl;
                //     if(toCheck[adjid].find(elid)!=toCheck[adjid].end())
                //     {
                //         E2A_set[elid].insert(adjid);
                //     }
                // }
        ...
        std::vector<int> adjv;
        std::copy(AdjElem.begin(), AdjElem.end(), std::back_inserter(adjv));
...
void PartObject::buildExtendedAdjacencyDataReFactor(MPI_Comm comm, 
                                           std::map<int,std::vector<double> > ghosts, int max_layers)
{
    ...
    for(auto elid : m_ElemSet)
    {
        std::set<int> AdjElem;
        std::queue<std::pair<int, int>> q; // {element, current layer}
        q.push({elid, 0});

        while(!q.empty())
        {
            auto [curr_id, layer] = q.front(); q.pop();
            ...
            {
                AdjElem.insert(curr_id);
            } 

            if(layer < max_layers)
            {
                for(auto adj_id : m_Elem2Elem[curr_id])
                {
                    if(m_Elem2Elem.count(adj_id) && ghosts.count(adj_id)==0 && AdjElem.count(adj_id)==0)
                        q.push({adj_id, layer + 1});
                }
            }
        }

        std::vector<int> adjv(AdjElem.begin(), AdjElem.end());
        m_Elem2AdjElem_ReFactor[elid] = adjv;
    }
}
                                            ...
                                            std::map<int,std::vector<double> > ghosts)
{
    ...
    // std::map<int,std::set<int> > Elems2Check;
    // std::cout << "m_Elem2Vert " << m_Elem2Vert.size() << " " << m_Elem2Elem.size() << std::endl;
    for(its=m_LocElem.begin();its!=m_LocElem.end();its++)
    {
        int elid        = *its;
        std::set<int> AdjElem;
        // if layer = 1:
        int nadj1 = m_Elem2Elem[elid].size();
        std::vector<double> Vijk = m_Elem2Centroid[elid];

        for(int j=0;j<nadj1;j++)
        {
            int adj1_id  = m_Elem2Elem[elid][j];
            int fadj   = m_Elem2Face[elid][j];

            if(m_Elem2Elem.find(adj1_id)!=m_Elem2Elem.end()
                && ghosts.find(adj1_id)==ghosts.end())
            {
                AdjElem.insert(adj1_id);

                // if layer = 2:
                int nadj2  = m_Elem2Elem[adj1_id].size();

                for(int j=0;j<nadj2;j++)
                {
                    int adj2_id  = m_Elem2Elem[adj1_id][j];
                    if(m_Elem2Elem.find(adj2_id)!=m_Elem2Elem.end()
                        && ghosts.find(adj2_id)==ghosts.end())
                    {
                        AdjElem.insert(adj2_id);

                        // if layer = 3:
                        int nadj3  = m_Elem2Elem[adj2_id].size();

                        for(int j=0;j<nadj3;j++)
                        {
                            int adj3_id  = m_Elem2Elem[adj2_id][j];

                            if(m_Elem2Elem.find(adj3_id)!=m_Elem2Elem.end()
                                && ghosts.find(adj3_id)==ghosts.end())
                            {
                                AdjElem.insert(adj3_id);
...
std::map<int,std::vector<int> > PartObject::getExtendedAdjacencyData()
{
    return m_Elem2AdjElem;
}

std::map<int,std::vector<int> > PartObject::getExtendedAdjacencyDataReFactor()
{
    return m_Elem2AdjElem_ReFactor;
}
