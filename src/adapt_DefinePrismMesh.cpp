...
//    int nNonSharedFaces          = nLocalFaces-owned_faces[world_rank];
    ...
    int iVshared2         = distLocalVerts->getNel()-v2r.size();
    
    std::cout << "check shared verts = " << world_rank << " " << iVshared << " " << iVshared2 << " " << distLocalVerts->getNel() << " v2r => " << v2r.size() << " " <<nNonSharedVertsTot << " " << shellvert2ref_glob.size() << std::endl;
...
//                                Vfaces.push_back(Vf);
                                ...
                                {
                                    if(NonSharedVertsOwned.find(fn_tag[n])==NonSharedVertsOwned.end())
                                    {
...
//                            }
    ...
    std::cout << " lhp vs rhp sizes BEFORE " << lhp.size() << " " << rhp.size() << " " << bcFace2Node_prism.size() << std::endl;
    ...
    std::cout << " lhp vs rhp sizes AFTER " << lhp.size() << " " << rhp.size() << " " << bcFace2Node_prism.size() << " " << lhp.size() - rhp.size() << " " << shFid2el_rh.size() << " " << adde << " " << sharedFace2Node_prism.size() <<  std::endl;
...
//
    ...
    std::cout << "check lengthfs = " << nUniqueVerts_prisms << " " <<NonSharedVertsOwned.size() << " " << SharedVertsOwned.size() << " " << tag2glob_prism.size() << std::endl;
            ...
            if(f2r.find(gfid)!=f2r.end() && ref != 13)
            {
                if(f2r[gfid]==world_rank)
                {
                    ...
                    if(glf_map.find(gfid)==glf_map.end())
                    {
                        ...
                        if(lEl == testElem)
                        {
                            std::cout << "lhp[gfid] shared " << gfid << " " << ref << " " << orient0 << std::endl;
                        }
                    ...
                    else
                    {
                        ...
                        if(lEl == testElem)
                        {
                            std::cout << "rhp[gfid] shared " << gfid << " " << ref << " " << orient0 << std::endl;
                        }
