                                                 ...
                                                 std::map<int,std::vector<double> >& elements2data_update)
{
    ...
    for(m=0;m<loc_r_elem.size();m++)
    {
        ...
        for(int p=0;p<nvPerEl;p++)
        {
            ...
            Pijk[p*3+0] = LocalVertsMap[glob_v][0];
            Pijk[p*3+1] = LocalVertsMap[glob_v][1];
            Pijk[p*3+2] = LocalVertsMap[glob_v][2];
                                             ...
                                             std::map<int,std::vector<double> >& elements2data_update_output)
{
    ...
    ScheduleObj* adj_schedule = DoScheduling(adj_elements, comm);
    ...
    for(q=0;q<size;q++)
    {
        ...
        {
            int i=0;
            for (it = adj_elements.begin(); it != adj_elements.end(); it++)
            {
    ...
    for(itv=reqstd_adj_ids_per_rank.begin();itv!=reqstd_adj_ids_per_rank.end();itv++)
    {
        ...
        for(int j=0;j<itv->second.size();j++)
        {
            ...
            {
                f_id = element2faces[adj_id][k];
                send_adj_faces_IDs[dest].push_back(f_id);
                e_id = element2element[adj_id][k];
    ...
    //std::cout << " rank checkcheckcheck " << rank << " " << adj_elements_vec.size() << " :: " << elements2elements_update_output.size() << std::endl;

    int before = elements2elements_update_output.size();

    
    //std::cout << "before22222 " << rank << " :: " << elements2verts_update_output.size() << " " << elements2faces_update_output.size() << " " << elements2elements_update_output.size() << " " << elements2data_update_output.size() << std::endl;  
...
//      ElemPart->setVal(m+o,0,el_id);
        ...
        for(int p=0;p<Nv;p++)
        {
            ...
            loc_v  = o_gvertex2lvertex[glob_v];
            Padj[p*3+0] = LocalVertsMap[glob_v][0];
            Padj[p*3+1] = LocalVertsMap[glob_v][1];
            Padj[p*3+2] = LocalVertsMap[glob_v][2];
        ...
        if(elements2elements_update_output.find(el_id)!=elements2elements_update_output.end())
        {
            //std::cout << "its here already " << std::endl;
        }
        else
        {
    ...
    // if(rank == 18)
    // {
    //     std::cout << "LocAndAdj_Elem " << LocAndAdj_Elem.size() << std::endl;
    // }

    //std::cout << "after " << rank << " :: " << adjEl2Face.size() << " " << elements2verts_update_output.size() << " " << elements2faces_update_output.size() << " " << elements2elements_update_output.size() << " " << elements2data_update_output.size() << std::endl;  

    
     //std::cout << "after2222 " << rank << " :: " << elements2verts_update_output.size() << " " << elements2faces_update_output.size() << " " << elements2elements_update_output.size() << " " << elements2data_update_output.size() << std::endl;  
...
std::map<int,std::set<int> > RepartitionObject::getExtendedAdjacencyDataV2(std::map<int,std::vector<double> > &ghostface_vrt)
{
                        ...
                        && adjadj!=elid)
                    {
                        ...
                        && adjadjadj!=elid)
                            ...
                            {  
                                ...
                                {
                                    int gvid    = face2verts_update[fadjadjadj][s];
