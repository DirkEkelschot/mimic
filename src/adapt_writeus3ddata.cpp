                                ...
                                std::vector<int> &zdefs_new)
{
        ...
        if(Nbfp!=0)
        {
            ...
            for(int q=0;q<Nbfp;q++)
            {
                ...
                for(int g=0;g<nppf;g++)
                {
                    ...
                    else if(SharedVertsNotOwned_P.find(tagV)!=SharedVertsNotOwned_P.end() &&
                            glob_trace_verts.find(tagV)==glob_trace_verts.end())
                    {
                        int globid = SharedVertsNotOwned_P[tagV];
                        fce[g]     = globid;
                    }
                    else if(glob_trace_verts.find(tagV)!=glob_trace_verts.end())
                    {
    ...
    int ftott = (int)ifn_T.size()/8;
    int ftotp = (int)ifn_P.size()/8;
    int nNonSharedVertsOwned_P                  = NonSharedVertsOwned_P.size();//prism_repart->getNonSharedVertsOwned().size();
    int nSharedVertsOwned_P                     = SharedVertsOwned_P.size();//prism_repart->getSharedVertsOwned().size();
    ...
    DistributedParallelState* distTetra         = new DistributedParallelState(new_tetrahedra_T.size(),comm);
    DistributedParallelState* distPrism         = new DistributedParallelState(nLocPrismElem,comm);
