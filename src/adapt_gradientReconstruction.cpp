                                                           ...
                                                           int extrap)
{
    ...
    std::map<int,std::vector<int> > Elem2AdjElem                   = RePa->getExtendedAdjacencyData();
    ...
    for(its=m_ElemSet.begin();its!=m_ElemSet.end();its++)
    {
            ...
            std::vector<int> adjel         = Elem2AdjElem[elID];
            int g                       = 0;
            ...
            if(nadj>=9)
            {
                ...
                for(itv=adjel.begin();itv!=adjel.end();itv++)
                {
                        ...
                        int extrap)
{
    ...
    std::map<int,std::map<int, double> > Elem2ConnectedVertDist = RePa->getElem2ConnectedVertMap();
    std::map<int,std::vector<double> > Elem2Centroid            = RePa->getElem2CentroidData();
    std::map<int,std::vector<int> > Elem2AdjElem                = RePa->getExtendedAdjacencyDataReFactor();
    ...
    // std::cout << " total large matrices on rank: " << world_rank << " " << nnine << " vs " << bnine << std::endl;
