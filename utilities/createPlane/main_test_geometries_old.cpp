...
void createSimpleAirplane(const std::string& filename) {
    std::cout << "Creating simple airplane geometry..." << std::endl;
    
    // Fuselage (elongated ellipsoid made from scaled sphere)
    TopoDS_Shape fuselage = BRepPrimAPI_MakeSphere(gp_Pnt(0, 0, 0), 1.0).Shape();
    // Scale it to make elongated (we'll use cylinder + cones instead for simplicity)
    ...
    // Wings (flat boxes)
    TopoDS_Shape leftWing = BRepPrimAPI_MakeBox(gp_Pnt(-1, 0.5, -0.1),
                                                  gp_Pnt(1, 4, 0.1)).Shape();
    TopoDS_Shape rightWing = BRepPrimAPI_MakeBox(gp_Pnt(-1, -4, -0.1),
                                                   gp_Pnt(1, -0.5, 0.1)).Shape();
    
    // Tail wings (horizontal stabilizer)
    TopoDS_Shape leftTailWing = BRepPrimAPI_MakeBox(gp_Pnt(-3.5, 0.3, -0.05),
                                                      gp_Pnt(-2.5, 1.5, 0.05)).Shape();
    TopoDS_Shape rightTailWing = BRepPrimAPI_MakeBox(gp_Pnt(-3.5, -1.5, -0.05),
                                                       gp_Pnt(-2.5, -0.3, 0.05)).Shape();
    
    // Vertical stabilizer
    TopoDS_Shape vertStabilizer = BRepPrimAPI_MakeBox(gp_Pnt(-3.5, -0.05, 0),
                                                        gp_Pnt(-2.5, 0.05, 1.2)).Shape();
