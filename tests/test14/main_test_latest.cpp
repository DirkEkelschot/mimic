...
struct newNumberingNodesFaces
{
    ...
    std::map<int,int> globalV2localV;
    Array<double>* xcn;
...
//    int nNonSharedFaces          = nLocalFaces-owned_faces[world_rank];
    ...
    std::map<int,int> lE2gE;
    std::map<int,int> shF2El;
    
    for(prit=elements.begin();prit!=elements.end();prit++)
    {
        int gEl  = prit->first;
        int rank = part_global->getVal(gEl,0);
        int lEl  = ElementDistr->getOffsets()[rank]+u;
        ...
        for(int q=0;q<ief_part_map[gEl].size();q++)
        {
            ...
            if(f2r.find(gfid)!=f2r.end())
            {
            	if(f2r[gfid]==world_rank)
            	{
					...
					if(el0==gEl)
					{
						...
						colorFh[r1].push_back(lfid2);
					}
					else if(el1==gEl)
					{
						colorRh[r0].push_back(el0);
						colorFh[r0].push_back(lfid2);
					}
            	}
            }
            else
            {
                if(glf_set.find(gfid)==glf_set.end())
                {
                    ...
                    if(ref!=13 && ref!=2)
                    {
                    	ref2bcface[ref].push_back(lbfid);
                    }
                    lbfid              = lbfid + 1;
                }
                else
    ...
    for(rcvit=adj_ids.begin();rcvit!=adj_ids.end();rcvit++)
    {
    	...
    	for(int q=0;q<nelem;q++)
    	{
        	...
        	if(shFid2el_rh.find(tag)==shFid2el_rh.end())
        	{
    ...
    for(pritm=glf_map.begin();pritm!=glf_map.end();pritm++)
    {
        ...
        if(ref != 13 && ref != 2)
        {
            bcFace2Node_prism[lfid]     = fn;
        }
        if(ref == 2)
        {
            intFace2Node_prism[lfid]    = fn;
        }
        if(ref == 13)
        {
            shellFace2Node_prism[lfid]  = fn;
        }
    ...
    for(pritm=sharedFmap.begin();pritm!=sharedFmap.end();pritm++)
    {
        gfid = pritm->first;
        lfid = pritm->second;
    ...
    //
            sharedFace2Node_prism[lfid] = fn;
            pface2ref[lfid]             = ref;
            face2node[lfid]             = fn;
            
            rhp[lfid] 					= shFid2el_rh[lfid];
        }
    }
    ...
    Array<double>* xcn_tmp = new Array<double>(gl_map.size(),3);
    int lvert = 0;
    std::map<int,int> globalV2localV;
    std::map<int,int> localV2globalV;
    for(itred=gl_map.begin();itred!=gl_map.end();itred++)
    {
    	int lvid  = itred->second;
    	int tag   = itred->first;
    	
    	int lc    = tag2locV[tag];
    	
    	double xp = locVerts[lc]->x;
    	double yp = locVerts[lc]->y;
    	double zp = locVerts[lc]->z;
    	
    	xcn_tmp->setVal(lvert,0,xp);
    	xcn_tmp->setVal(lvert,1,yp);
    	xcn_tmp->setVal(lvert,2,zp);
    	
    	globalV2localV[lvid] = lvert;
    	localV2globalV[lvert] = lvid;
    ...
    nnf->localV2globalV = localV2globalV;
    nnf->globalV2localV = globalV2localV;
...
//                                                                                hess_vmap, comm);
    ...
    std::map<int,int> shellvert2ref                 = tetra_distri->GetShellVert2RefMap();
    ...
    Array<int>* ifnOUT_prism = new Array<int>(int_face2node_prism.size()+shared_face2node_prism.size(),7);
    ...
    for(itmm=ushell.begin();itmm!=ushell.end();itmm++)
    {
        ...
        if(shellface2vertref.find(fhyb)!=shellface2vertref.end())
        {
            ...
            //shell_prism2vertref[EprismNew] = shellface2vertref[fhyb];
            foundU++;
        }
        nshell++;
    }
    ...
    std::cout << "FoundU =============>>>>>>>>>>>  " << shellface2vertref.size() << " " << vertref2shell_prism.size() << " " << nshell << " " << ushell.size() << std::endl;
...
//        }
        ...
        if(refer == 13)
        {
            if(shell_tet2hybF.find(faceID)!=shell_tet2hybF.end())
            {
                ...
                else
                {
                    ...
                    if(sft_t==sf)
                    {
                        if(vertref2shell_prism.find(sft_t)!=vertref2shell_prism.end())
                        {
                            shelfound++;
    ...
    std::cout << "faceUvids " << faceUvids.size() <<  " "  << c13 << " " << suc << " " << buggi << " " << shelfound  << std::endl;
...
//    }
    ...
    std::cout << "found Ref " << foundRref << " " << minrf << " " << maxrf << std::endl;
...
//           
        ...
        std::cout << world_rank << " PMMG_ShellFaces size  " << PMMG_ShellFaces.size() << " prismFound-> " << prismFound << " " << faceCovered << " " <<  std::endl;
...
//        myfile4.close();
        ...
        int curElID = TetraOUT_offsets[world_rank]+1+nPrisms;
        ...
        for( prit=int_face2node_prism.begin();prit!=int_face2node_prism.end();prit++)
        {
            int lfid  = prit->first;
            int fref  = ifref[lfid];
            int gfid  = locface2globface[lfid];
            int npf   = int_face2node_prism[lfid].size();
            ifnOUT_prism->setVal(fptot,0,npf);
            
            for(int g=0;g<npf;g++)
            {
                ifnOUT_prism->setVal(fptot,g+1,int_face2node_prism[lfid][g]);
            }
            ...
            ifnOUT_prism->setVal(fptot,5,rhp[lfid]);
            ifnOUT_prism->setVal(fptot,6,lhp[lfid]);
        ...
        for( prit=shared_face2node_prism.begin();prit!=shared_face2node_prism.end();prit++)
        {
            ...
            int fref  = ifref[lfid];
            int gfid  = locface2globface[lfid];
            int npf   = shared_face2node_prism[lfid].size();
            
            ifnOUT_prism->setVal(fptot,0,npf);
            
            for(int g=0;g<npf;g++)
            {
                ifnOUT_prism->setVal(fptot,g+1,shared_face2node_prism[lfid][g]);
            }
        ...
        std::cout << world_rank << " prism faces " << fptot << std::endl;
                ...
                if(facemap.find(Face)==facemap.end())
                {
                    ...
                    if(PMMG_Face2Ref.find(Face) != PMMG_Face2Ref.end())
                    {
                    	...
                    	if(FaceRef == 13)
                    	{
							...
							{
								rhshell[fid] = vertref2shell_prism[Face];
							}
...
//                    delete n0;
        ...
        std::cout << "fteet " << ftot << " on rank " << world_rank << std::endl;
        ...
        for(int i=0;i<Nt_BCs;i++)
        {
            if(bcsToT.find(BCs_arr[i])==bcsToT.end())
            {
                ...
                if(world_rank == 0)
                {
                    std::cout << "bcid = "<< " " <<  BCs_arr[i] << std::endl;
...
//			if(bc_id == 10)
//			{
//	            std::cout << "===++++ " << bc_id << " " << Nbft << " " << Nbfp << " " << nlbc[i] << " " << NelLoc_bci << " WRaDd = " << world_rank << " +++++===="<< std::endl;
//
//			}
            ...
            if(Nbfp!=0)
            {
                for(int q=0;q<Nbfp;q++)
				{
					...
					for(int s=0;s<nppf;s++)
					{
						ifn_bc_i->setVal(fbc,s+1,bc_face2node_prism[bcface][s]);					
					}
...
//            
            if(Nbft!=0 )
            {
            	//std::cout << " DOUBLE CHECK " << Nbft << " " <<  Nbfp << " "<< bc_id << " " << world_rank << " " << fbc << std::endl;
