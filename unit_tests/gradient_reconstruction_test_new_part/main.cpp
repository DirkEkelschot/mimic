...
void SetAnalyticalSolution(PartObject* tetra_repart,
                            int Nel_glob,
                            ...
                            std::map<int,std::vector<double> > &gbMap_dU2dxz)
{
    ...
    std::map<int,std::vector<int> > element2vert_map    = tetra_repart->getElem2VertMap();
    std::map<int,std::vector<int> > element2element_map = tetra_repart->getElem2ElemMap();
    std::map<int,std::vector<int> > element2face_map    = tetra_repart->getElem2FaceMap();
    std::map<int,std::vector<int> > face2vert_map       = tetra_repart->getFace2VertMap();
    std::map<int,std::vector<double> > locVs            = tetra_repart->getLocalVertsMap();
    std::set<int> ElemSet                               = tetra_repart->getLocalElemSet();

    // std::cout << " locVs " << locVs.size() << " " << Nel_glob << " " << element2vert_map.size() << " " << ElemSet.size() << std::endl;
    ...
    for(itmii=element2vert_map.begin();itmii!=element2vert_map.end();itmii++)
    {
        ...
        for(int j=0;j<nadj;j++)
        {
            ...
            if(adjID>=Nel_glob)
            {
                ...
                if(face2vert_map.find(fid)==face2vert_map.end())
                {
                    std::cout << "Not here " << fid << std::endl;
                }
...
int main(int argc, char** argv)
{
    ...
    for(itg=grids.begin();itg!=grids.end();itg++)
    {
        ...
        start = clock();
        PartObject* tetra_repart = new PartObject(meshRead, 
                                                tetras_e2v, 
                                                tetras_e2f,
                                                tetras_e2e,
                                                tetras_data,
        ...
        // PrepareAdaption* adaptionObject = new PrepareAdaption(tetra_repart, 
        //                                                       comm, 
        //                                                       m_Elem2Face,
        //                                                       m_Elem2Vert,
        //                                                       m_Face2Vert,
        ...
        if(world_rank == 0)
        {
            std::cout << std::setprecision(16) << "Finished running buildUpdatedVertexAndFaceNumbering:                                 " << std::fixed
            << dur_max;
        ...
        if(world_rank == 0)
        {
            std::cout << std::setprecision(16) << "Finished running buildInteriorSharedAndBoundaryFaceMaps:                             " << std::fixed
            << dur_max;
            std::cout << " sec " << std::endl;
        }
        ...
        */
        ...
        SetAnalyticalSolution(tetra_repart, nGlob,
                            U_map, Usol,
                            gbMap, 
                            gbMap_dUdx, gbMap_dUdy, gbMap_dUdz, 
                            gbMap_dUdXi, gbMap_dU2dx2, gbMap_dU2dxy, gbMap_dU2dxz);
        
        tetra_repart->buildExtendedAdjacencyData(comm,gbMap);
        ...
        std::map<int,std::vector<double> > tetra_grad_extended = ComputedUdx_LSQ_LS_US3D(tetra_repart, gbMap, meshRead->nElem,0,1,comm,0);
        ...
        std::map<int,std::vector<double> > dU2dx2_extended = ComputedUdx_LSQ_LS_US3D(tetra_repart,
                                                                            gbMap_dUdx,
                                                                            meshRead->nElem,
                                                                            0,
                                                                            1,
        ...
        std::map<int,std::vector<int> > face2element_map    = tetra_repart->getFace2ElemMap();
        ...
        std::map<int,std::vector<double> > loc_data_t       = tetra_repart->getElem2DataMap();
        std::map<int,std::vector<int> > gE2lV_t             = tetra_repart->getElem2LocalVertMap();
